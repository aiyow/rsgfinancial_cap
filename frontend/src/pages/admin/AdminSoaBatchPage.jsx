  ...
  return (
    <DashboardLayout title="Forwarded billing batch" description="Open any read-only Statement of Account in this batch.">
      ...
      <Panel title={`${bills.length} Statements of Account`} description={`${publishedCount} published to Residents | ${unpublished.length} still hidden`}>
        ...
        <div className="mb-4 rounded-xl border border-slate-100 bg-slate-50 p-4"><p className="mb-3 text-sm font-black text-slate-800">Filter Statements of Account</p><div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_170px_180px]"><label className="text-xs font-bold text-slate-600">Search unit or payer<input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1) }} placeholder="e.g. 401 or owner name" className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-normal" /></label><label className="text-xs font-bold text-slate-600">Publication<select value={publishFilter} onChange={(event) => { setPublishFilter(event.target.value); setPage(1) }} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-normal"><option value="ALL">All SOAs</option><option value="PUBLISHED">Published</option><option value="HIDDEN">Hidden</option></select></label><label className="text-xs font-bold text-slate-600">Email delivery<select value={deliveryFilter} onChange={(event) => { setDeliveryFilter(event.target.value); setPage(1) }} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-normal"><option value="ALL">All statuses</option><option value="SENT">Sent</option><option value="FAILED">Failed</option><option value="PENDING">Pending</option><option value="NO_RECIPIENTS">No recipients</option></select></label></div></div>
