...
export default function AdminSoaBillPage() {
  ...
  async function saveReferences(event) {
    ...
    try {
      ...
      setBill(data.bill); setReferences({ officialReceiptNumber: data.bill.officialReceiptNumber || '', invoiceNumber: data.bill.invoiceNumber || '', paymentNote: data.bill.paymentNote || '' }); setNotice({ error: '', message: data.message })
    } catch (error) { setNotice({ error: error.message, message: '' }) } finally { setBusy(false) }
  }
  ...
  const returnTo = openedFromBillingErrors ? '/admin/billing-errors' : bill ? `/admin/soa/batches/${bill.billingPeriodId}` : '/admin/soa'
