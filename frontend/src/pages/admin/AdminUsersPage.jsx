...
function roleLabel(role) {
  return { ADMIN: 'Admin', COLLECTOR: 'Billing Associate', RESIDENT: 'Resident' }[role] || role
}
  ...
  return (
    <DashboardLayout title="User management" description="Manage Admin, Billing Associate, and Resident accounts.">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div><h1 className="text-2xl font-black tracking-tight text-[var(--ink)]">User management</h1><p className="mt-1 text-sm text-[var(--muted)]">{users.length} accounts · {users.filter((user) => user.isActive).length} active · {users.filter((user) => user.approvalStatus === 'PENDING').length} awaiting approval</p></div>
      ...
      <section className="overflow-visible rounded-2xl border border-[var(--border)] bg-white shadow-sm">
        <div className="grid gap-3 border-b border-[var(--border)] p-4 md:grid-cols-[minmax(260px,1fr)_180px_180px_auto] md:items-center">
          ...
          <FilterPopover label="Role" value={roleFilter} open={roleMenuOpen} onToggle={() => { setRoleMenuOpen((current) => !current); setStatusMenuOpen(false) }} onSelect={(value) => { setRoleFilter(value); setRoleMenuOpen(false) }} options={[{ value: 'ALL', label: 'Role: all' }, { value: 'ADMIN', label: 'Admin' }, { value: 'COLLECTOR', label: 'Billing Associate' }, { value: 'RESIDENT', label: 'Resident' }]} />
          <FilterPopover label="Status" value={statusFilter} open={statusMenuOpen} onToggle={() => { setStatusMenuOpen((current) => !current); setRoleMenuOpen(false) }} onSelect={(value) => { setStatusFilter(value); setStatusMenuOpen(false) }} options={[{ value: 'ALL', label: 'Status: all' }, { value: 'PENDING', label: 'Awaiting approval' }, { value: 'ACTIVE', label: 'Active' }, { value: 'INACTIVE', label: 'Inactive' }]} />
