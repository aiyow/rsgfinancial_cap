  ...
  return (
    <DashboardLayout title="Monthly billing" description="Create a draft period, validate the Collector workbook, and generate unit bills.">
      ...
      <Panel title="2. Upload and validate readings" description="Required columns: UNIT, PREVIOUS, and PRESENT. Server calculations override spreadsheet formulas.">
        ...
        {preview && (
          <div className="mt-5">
            <div className="mb-3 flex flex-wrap items-center justify-between gap-3 text-sm"><p><strong>{preview.summary.rowCount}</strong> spreadsheet rows | <strong>{preview.summary.unitCount}</strong> database units | <strong>{preview.summary.warningCount}</strong> warnings</p><button type="button" disabled={!preview.valid || busy} onClick={importReadings} className={primaryClass}>Confirm import</button></div>
